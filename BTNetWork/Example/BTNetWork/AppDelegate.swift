...
extension AppDelegate: NetWorkHandleProtocol {
    ...
    func signatureDic(_ param:[String:Any]?) -> [String: Any] {
        ...
        let NetKey = "sierra_2017"
        
        var tempParam:[String: Any] = param ?? [:]
        
        //时间戳
        let date = Date(timeIntervalSinceNow: 0)
        let seconds:TimeInterval = date.timeIntervalSince1970
        let secondStr = String(format: "%.0f", seconds)
        tempParam["timestamp"] = secondStr
        
        let concestr = String(format: "%@:%@", secondStr,NetKey)
        tempParam["noncestr"] = concestr
